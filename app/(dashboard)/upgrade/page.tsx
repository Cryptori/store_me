...
const FREE_FEATURES = ['50 produk', 'Kasir & transaksi harian', '10 pelanggan', 'Kelola hutang', 'Laporan harian']
const PRO_FEATURES = ['Produk unlimited', 'Pelanggan unlimited', 'Laporan bulanan', 'Export PDF', 'Multi-user kasir', 'Priority support', 'Semua fitur FREE']
    ...
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).snap) {
        ;(window as any).snap.pay(data.token, {
          ...
          onPending: () => {
            window.location.href = `/upgrade/pending`
          },
          onError: () => {
    ...
    <div className="p-6 flex flex-col items-center justify-center min-h-[60vh]">
      ...
      {store?.pro_expires_at && (
        <p className="text-[#64748b] text-sm mt-2">
          Aktif hingga {new Date(store.pro_expires_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
        </p>
      )}
    </div>
  )
